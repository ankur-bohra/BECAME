...
def swag_parameters(module, params, no_cov_mat=True, restrict_params=[]):
    for name in list(module._parameters.keys()):
        if len(restrict_params) > 0 and name not in restrict_params:
            continue
...
class SWAG(torch.nn.Module):
    def __init__(
        self, base, no_cov_mat=True, max_num_models=0, var_clamp=1e-30, restrict_params=[], *args, **kwargs
    ):
    ...
    def get_param_precision_matrix(self, param_name):
        for module, name in self.params:
            if name == param_name:
                mean = module.__getattr__("%s_mean" % name)
                sq_mean = module.__getattr__("%s_sq_mean" % name)
                cov_mat_sqrt = module.__getattr__("%s_cov_mat_sqrt" % name)  # Float
                var = sq_mean - mean ** 2.0  # Not flat
                var = var.view(-1)

                # Sigma = 0.5 * (Diag(var) + (1/(K-1)) * D_hat^T D_hat)
                sigma_diag = torch.diag(var)
                sigma_low_rank = cov_mat_sqrt @ cov_mat_sqrt.transpose(0, 1) / (self.max_num_models - 1)
                return 0.5 * (sigma_diag + sigma_low_rank)
        raise ValueError(f"Parameter {param_name} not found in SWAG model.")