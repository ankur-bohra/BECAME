...
class SVDAgent(Agent):
    ...
    def init_model_optimizer(self):
        ...
        try:
            ...
            # for joint multi-task training
            if self.config['multi_task']:
                cls_params = []
                for n in cls_params_all:
                    cls_params.extend(list(cls_params_all[n].parameters()))
        except:
            cls_params = list(cls_params_all['ALL'].parameters())
        ...
        if not self.config['early_stop']:
            ...
            # if training with early stopping, do not change the learning rate by scheduler
            self.model_scheduler = torch.optim.lr_scheduler.MultiStepLR(self.model_optimizer,
                                                                    milestones=self.config['schedule'][-1:],
                                                                    gamma=self.config['gamma'])

    def train_task(self, train_loader, val_loader=None, epochs=1, early_stop=False):

        self.train_model(train_loader, val_loader, epochs, early_stop=early_stop)


    def after_train(self, train_loader):
        self.task_count += 1
    ...
    def parameters_initilization(self):
