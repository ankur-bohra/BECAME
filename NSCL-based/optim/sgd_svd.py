...
class SGDSVD(Optimizer):  # noqa: D101
    def __init__(
        self,
        params: ParamsT,
        ...
        *,
        ...
        svd: Optional[bool] = False,
        thres: Optional[float] = 1.001,
    ):  # noqa: D107
        if fused or foreach:
            #! Ignore fused and foreach implementations for NSCL
            raise NotImplementedError("Only single-tensor SGD is adapted for NSCL updates")
        ...
        self.switch = True  # switch to turn on/off NSCL updates
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            #! Collect updates instead of applying them directly
            grads = sgd(
                params,
                grads,
            ...
            svd_enabled = group.get("svd", False)

            #! Apply NSCL modifications and parameter updates
            self.modify_and_apply_updates(
                params,
                grads,
                lr=group["lr"],
                has_sparse_grad=has_sparse_grad,
    ...
    def modify_and_apply_updates(
        ...
        *,
        ...
        r"""Modify the gradients as per NSCL and apply the updates to parameters.

        Args:
            params (list of Tensor): Parameters.
            grads (list of Tensor): Gradients.
            lr (float): Learning rate.
            has_sparse_grad (bool): Whether any of the gradients are sparse.
        """
        if len(params) == 0:
            return
        ...
        for i, param in enumerate(params):
            # NSCL projection:
            # if svd and len(self.transforms) > 0 and self.switch:
            ...
            #! Project grad to null space
            grad = grads[i]
            if svd_enabled and len(self.transforms) and self.switch:
                # Null space is available and projection is enabled
                # Ignore if transform not found for parameter?
                assert param in self.transforms, "Transform matrix not found for parameter."
                transform = self.transforms[param]
                grad = torch.mm(grad.view(grad.size(0), -1), transform).view_as(grad)

            # _single_tensor_sgd update
    ...
    # --------------------------------------------------------------------------------- #
    #                            NSCL updates triggered by the agent                    #
    # --------------------------------------------------------------------------------- #

    def get_eigens(self, fea_in):
        """Compute the eigenvalues and eigenvectors for parameters given input features."""
        for group in self.param_groups:
            svd = group['svd']
            if svd is False:
                continue
